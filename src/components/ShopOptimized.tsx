...
import { ArrowLeft, Gift, Star, Check, ShoppingBag, Award, Shield } from "lucide-react";
...
// Optimized image imports with preloading
...
const generateRandomRating = () => Number((Math.random() * (5 - 4) + 4).toFixed(1));
const generateRandomReviews = () => Math.floor(Math.random() * (300 - 50) + 50);
...
const ShopOptimized = memo(() => {
  ...
  // Initialize products with processed data
  useEffect(() => {
  ...
  const handleSelectGift = useCallback((product: Product) => {
...
ShopOptimized.displayName = "ShopOptimized";
